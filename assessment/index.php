<?php
// Dynamic Title based on GET parameter
$gradeParam = isset($_GET['grade']) ? $_GET['grade'] : 'Third Grade';

// Map friendly URLs or IDs to display names if needed
$gradeMap = [
    'pre-k' => 'Pre-K',
    'k' => 'Kindergarten',
    '1' => 'First Grade',
...
// Normalize key
$gradeKey  = strtolower($gradeParam);
$gradeName = isset($gradeMap[$gradeKey]) ? $gradeMap[$gradeKey] : $gradeParam;

$pageTitle       = "$gradeName Assessment | Hesten's Learning";
$pageDescription = "Practice your $gradeName skills in Math, Reading, Science, and Social Studies.";
...
<header
    ...
    <div class="container mx-auto">
        ...
        <h1 class="text-3xl md:text-5xl font-extrabold leading-tight mb-4 dropdown-shadow">
            <?php echo $gradeName; ?> Knowledge Check
        </h1>
        ...
        <?php
        // Back link logic
        $levelLink = '/Level/e.php'; // Default
        if (strpos($gradeName, 'Pre-K') !== false)
            $levelLink = '/Level/a.php';
        if (strpos($gradeName, 'Kindergarten') !== false)
            $levelLink = '/Level/b.php';
        if (strpos($gradeName, 'First') !== false)
            $levelLink = '/Level/c.php';
        if (strpos($gradeName, 'Second') !== false)
            $levelLink = '/Level/d.php';
        if (strpos($gradeName, 'Third') !== false)
            $levelLink = '/Level/e.php';
        if (strpos($gradeName, 'Fourth') !== false)
            $levelLink = '/Level/f.php';
        if (strpos($gradeName, 'Fifth') !== false)
            $levelLink = '/Level/g.php';
        ?>

        <a href="<?php echo $levelLink; ?>"
            class="inline-block mt-6 px-6 py-2 bg-white text-blue-700 rounded-full font-bold hover:bg-blue-50 transition-colors shadow-lg">
...
<div class="container mx-auto px-4 pb-12">
    ...
    <input type="hidden" id="force-grade" value="<?php echo $gradeName; ?>">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                ...
                <div class="space-y-2">
                    ...
                    <button onclick="filterQuestions('Math')"
                        class="w-full text-left px-4 py-3 rounded-lg bg-gray-50 dark:bg-gray-700 hover:bg-blue-50 dark:hover:bg-blue-900/30 hover:text-blue-600 transition-colors font-medium border border-transparent focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
                        <i class="fas fa-calculator w-6 text-center mr-2 text-blue-500"></i> Math
                    </button>
            ...
            <div
                ...
                <div class="flex justify-between items-center mb-6 border-b border-gray-100 dark:border-gray-700 pb-4">
                    <div>
                        ...
                        <div id="question-count" class="text-4xl font-black text-gray-900 dark:text-white">1<span
                                class="text-xl text-gray-400 font-medium">/10</span></div>
                    </div>
                ...
                <!-- Hint Modal (Inline) -->
                <div id="hintText"
                    class="hidden mt-4 bg-yellow-50 dark:bg-yellow-900/30 p-4 rounded-lg border-l-4 border-yellow-400 text-yellow-800 dark:text-yellow-200 text-sm">
...
<script>
    // Initialize with the dynamic grade name
    document.addEventListener('DOMContentLoaded', () => {
        // Read the PHP-injected grade name from the hidden input or variable
