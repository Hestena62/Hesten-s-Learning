...
window.addXP = function(amount, reason) {
    userData.xp += amount;
    const nextLevelXP = window.getNextLevelXP(userData.level);
...
function injectLevelHUD() {
    ...
    if (!nav || document.getElementById('hl-level-hud')) return;
    ...
    hud.innerHTML = `
        ...
        <div class="hidden sm:block w-24 h-2 bg-black/10 rounded-full overflow-hidden">
            <div id="hud-xp-bar" class="h-full bg-gradient-to-r from-primary to-secondary transition-all duration-500" style="width: ${(userData.xp / window.getNextLevelXP(userData.level)) * 100}%"></div>
        </div>
        <div class="text-[10px] font-bold text-text-default uppercase tracking-widest opacity-70 cursor-help" id="hud-xp-text" title="Next level at ${window.getNextLevelXP(userData.level)} XP">${userData.xp} XP</div>
    `;
    
    const searchForm = nav.querySelector('form');
    if (searchForm) nav.insertBefore(hud, searchForm);
    else nav.appendChild(hud);
}
...
function updateHUD() {
    ...
    if (xpBar) xpBar.style.width = `${(userData.xp / window.getNextLevelXP(userData.level)) * 100}%`;
    if (xpText) {
        xpText.textContent = `${userData.xp} XP`;
        xpText.title = `Next level at ${window.getNextLevelXP(userData.level)} XP`;
    }
}
