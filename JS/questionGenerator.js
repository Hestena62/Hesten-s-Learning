...
function loadQuestions(grade) {
    currentQuestions = generateQuestionsForGrade(grade);
...
function generateQuestionsForGrade(grade) {
    const questions = [];
    const add = (q, opts, ans, hint, exp) => questions.push({question: q, options: opts, answer: ans, hint: hint, explanation: exp});

    if(grade === 'Pre-K' || grade === 'First Grade') {
        add("Which number comes after 5?", ["4", "6", "7", "3"], "6", "Count: 1, 2, 3, 4, 5...", "6 comes right after 5.");
        add("Which shape is round?", ["Square", "Triangle", "Circle", "Rectangle"], "Circle", "Think of a ball.", "A circle has no corners.");
        add("What color is the sky usually?", ["Green", "Blue", "Red", "Purple"], "Blue", "Look up outside!", "The sky appears blue due to sunlight scattering.");
        add("2 + 2 = ?", ["3", "4", "5", "22"], "4", "Use your fingers.", "2 plus 2 makes 4.");
    } 
    else if(grade === 'Second Grade' || grade === 'Third Grade') {
        add("5 x 5 = ?", ["10", "25", "55", "15"], "25", "Count by 5s five times.", "Multiplication is repeated addition.");
        add("Which is a noun?", ["Run", "Happy", "Cat", "Quickly"], "Cat", "Person, place, or thing.", "A cat is a thing (animal).");
        add("What do plants need to grow?", ["Candy", "Sunlight", "Music", "Toys"], "Sunlight", "It comes from the sky.", "Plants need sunlight for photosynthesis.");
    }
    else if(grade === 'Fourth Grade' || grade === 'Fifth Grade') {
        add("What is 1/2 as a decimal?", ["0.5", "0.2", "0.75", "1.2"], "0.5", "Think of half a dollar.", "1 divided by 2 is 0.5");
        add("Which planet is the Red Planet?", ["Venus", "Mars", "Jupiter", "Saturn"], "Mars", "It's the 4th planet.", "Mars has iron oxide dust giving it a red color.");
        add("30 ÷ 6 = ?", ["4", "5", "6", "7"], "5", "What times 6 equals 30?", "6 x 5 = 30.");
    }
    else {
        add("Solve for x: 2x = 10", ["2", "5", "8", "20"], "5", "Divide by 2.", "If 2x = 10, then x = 10/2.");
        add("What is the capital of France?", ["London", "Berlin", "Paris", "Rome"], "Paris", "Eiffel Tower location.", "Paris is the capital.");
        add("Chemical symbol for Gold?", ["Ag", "Au", "Fe", "Go"], "Au", "Latin 'Aurum'.", "Au stands for Aurum.");
        add("Value of Pi (approx)?", ["3.14", "4.13", "3.41", "3.00"], "3.14", "Circles related.", "Pi is approx 3.14159...");
    }

    for(let i=0; i<5; i++) {
        const a = Math.floor(Math.random()*10)+1;
        const b = Math.floor(Math.random()*10)+1;
        add(`What is ${a} + ${b}?`, [String(a+b), String(a+b+1), String(a+b-1), String(a+b+2)], String(a+b), "Add the numbers.", "Math fact.");
    }
